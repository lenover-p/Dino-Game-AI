...
class Dino_Pop():
    def __init__(self,popSize=1,sizex=-1,sizey=-1):
        self.indivs = []
        self.popSize = popSize
        for i in range(popSize):
            self.indivs.append(Dino(sizex, sizey))
            

    def CheckAllDead(self):
        for i in range(self.popSize):
            if self.indivs[i].isDead == False:
                return False
        return True
...
def DispStats(playerDino, pteras, cacti):
    print('Dino Score: ', playerDino.score)
    x=0
    y=0
    for c in cacti:
        x+=1
        print("Cactus ", x, ":")
        print("Cactus left: ",c.rect.left)
    for p in pteras:
        y+=1
        print("Ptera ", y, ":")
        print("Ptera left: ",p.rect.left)
        print("Ptera Alt: ",p.rect.centery)


    print('\n')

def GetInputs(gameInstance):
    cactiX = []
    peteraX = []
    peteraY = []
    for c in gameInstance.cacti:
        cactiX.append(c.rect.left)
    for p in gameInstance.pteras:
        peteraX.append(p.rect.left)
        peteraY.append(p.rect.centery)
    try:
        closestCactX = float(cactiX[0])
    except IndexError:
        closestCactX = 0.0
    try:
        closestPeteraX = float(peteraX[0])
        closestPeteraY = float(peteraY[0])
    except IndexError:
        closestPeteraX = 0.0
        closestPeteraY = 0.0
    return (closestCactX, closestPeteraX, closestPeteraY)
...
def gameplay():
    ...
    while not gameQuit:
        ...
        while not gameOver:
            ...
            #DispStats(playerDino, pteras, cacti)
...
def mult_dino_gameplay(pop):
    ...
    while not gv.gameQuit:
        while startMenu:
            pass
        while not gv.gameOver:
            if pygame.display.get_surface() == None:
                print("Couldn't load display surface")
                gv.gameQuit = True
                gv.gameOver = True
            ...
            if pop.CheckAllDead() == True:
                gv.gameOver = True
                #if max(pop.indivs.score) > high_score:
                    #high_score = max(pop.indivs.score)
...
class mult_dino_gameplay():
    def __init__(self, pop):
        ...
        self.retbutton_image,retbutton_rect = load_image('replay_button.png',35,31,-1)
        self.gameover_image,gameover_rect = load_image('game_over.png',190,11,-1)
    ...
    def loop(self):
        ...
        for dino in self.pop.indivs:
            if dino.isDead == False:
                dino.update()
                self.scb.update(dino.score)
            elif dino.isDead == True and dino.hidden == False:
                dino.rect.bottom = 0
                dino.hidden = True
...
def mult_dino_gameplay_setup(pop):
    global high_score
    gamespeed = 4
    startMenu = False
    gv.gameOver = False
